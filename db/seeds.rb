...
# aldi = Nokogiri::HTML(open(Store.first.url + products.first))
# kaufland = Nokogiri::HTML(open(Store.second.url + products.first))
# metro = Nokogiri::HTML(open(Store.third.url + products.first))

# puts "ALDI"
# aldi.search('.product-title').each do |name|
#   puts name.content.strip
# end

# puts "KAUFLAND"
# kaufland.search('.m-offer-tile__title').each do |name|
#   puts name.content.strip
# end
...
products.each do |product|
  Store.all.each do |store|
    ...
    file.search(store.name_class).each do |name|
      ...
      open_food = Nokogiri::HTML(URI.open("https://world.openfoodfacts.org/cgi/search.pl?search_terms=#{name}"))
    end
  end
end
